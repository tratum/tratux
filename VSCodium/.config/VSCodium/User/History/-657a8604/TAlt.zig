...
fn run_with_catch(a:i32, b:i32) void {
    const res = add(a, b) catch a; // WHat it does is that if the functiion returns an error then set the default value to a
    print("result is {}\n", .{res});
}

fn run_with_try(a:i32, b:i32) void {
    const res = try add(a, b); //  What try does is that when you call a function with try and if the function returns an error then it returns that error to the previous function
    print("result is {}\n", .{res});
}
