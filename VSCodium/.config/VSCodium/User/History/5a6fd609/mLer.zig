...
fn tuples() void {
    const tuple = .{ "Female", true };
    std.debug.print("Gender: {s}, Married: {}\n", .{ tuple[0], tuple[1] });
}
...
pub fn main() void {
    ...
    //printingEnums();
    printingStructs();
    const rec = rectangle{ .height = 20, .width = 30 };
    std.debug.print("The Rectangle with dimensions {d},{d} has {d}cm area", .{ rec.height, rec.width, rectangle.area() });
    inlineStruct();
    tuples();
}
