...
update_gtk_theming() {
  case "$1" in
    d)
      ...
      sed -i "28 c gsettings set org.gnome.desktop.interface gtk-theme 'Graphite-Dark'" "$SWAY_CONFIG"
      sed -i "29 c gsettings set org.gnome.desktop.interface icon-theme 'Win10Sur-dark'" "$SWAY_CONFIG"
      sed -i "30 c gsettings set org.gnome.desktop.interface color-scheme 'prefer-dark'" "$SWAY_CONFIG"
      cat > "$GTK3_CONFIG/settings.ini" <<EOL
      [Settings]
      gtk-theme-name=Graphite-Dark
      gtk-icon-theme-name=Win10Sur-dark
      gtk-cursor-theme-name=Bibata-Modern-Ice
      gtk-font-name=Afacad Flux Medium 18
      gtk-application-prefer-dark-theme=true
EOL
      cat > "$GTK4_CONFIG/settings.ini" <<EOL
      [Settings]
      gtk-theme-name=Graphite-Dark
      gtk-icon-theme-name=Win10Sur-dark
      gtk-cursor-theme-name=Bibata-Modern-Ice
...
EOL
  ;;
    ...
    l)
      ...
      sed -i "28 c gsettings set org.gnome.desktop.interface gtk-theme 'Graphite-Light'" "$SWAY_CONFIG"
      sed -i "29 c gsettings set org.gnome.desktop.interface icon-theme 'Win10Sur'" "$SWAY_CONFIG"
      sed -i "30 c gsettings set org.gnome.desktop.interface color-scheme 'prefer-light'" "$SWAY_CONFIG"
      cat > "$GTK3_CONFIG/settings.ini" <<EOL
      [Settings]
