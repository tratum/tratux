...
EOL
  ;;
    ...
    l)
      ...
      sed -i "7 c gsettings set org.gnome.desktop.interface gtk-theme 'Graphite-Light'" "$SWAY_CONFIG"
      sed -i "8 c gsettings set org.gnome.desktop.interface icon-theme 'Win10Sur'" "$SWAY_CONFIG"
      sed -i "9 c gsettings set org.gnome.desktop.interface color-scheme 'prefer-light'" "$SWAY_CONFIG"
      cat > "$GTK3_CONFIG/settings.ini" <<EOL
      [Settings]
