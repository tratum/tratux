...
update_gtk_theming() {
  case "$1" in
    d)
      ...
      sed -i "29 c gsettings set org.gnome.desktop.interface gtk-theme 'Graphite-Dark'" "$SWAY_CONFIG"
      sed -i "30 c gsettings set org.gnome.desktop.interface icon-theme 'Win10Sur-dark'" "$SWAY_CONFIG"
      sed -i "31 c gsettings set org.gnome.desktop.interface color-scheme 'prefer-dark'" "$SWAY_CONFIG"
...
EOL
  ;;
    ...
    l)
      ...
      sed -i "29 c gsettings set org.gnome.desktop.interface gtk-theme 'Graphite-Light'" "$SWAY_CONFIG"
      sed -i "30 c gsettings set org.gnome.desktop.interface icon-theme 'Win10Sur'" "$SWAY_CONFIG"
      sed -i "31 c gsettings set org.gnome.desktop.interface color-scheme 'prefer-light'" "$SWAY_CONFIG"
      cat > "$GTK3_CONFIG/settings.ini" <<EOL
      [Settings]
