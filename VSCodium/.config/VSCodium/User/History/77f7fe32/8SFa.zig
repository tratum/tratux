...
fn forLoops() void {
    ...
    // This will work the same with the 'continue' keyword     
}

fn whileLoops() void {
    print("The While Loop\n", .{});

    // While loops keeps on traversing until a condition is met. It's the same as the other laguages

    var dex:u8 = 0;
    print("While Loop Started\n", .{});
    while (true) {
        dex+=1;
        if (dex>10) break;  // if this line gets commented-out then the while loop will go on forever until it exhausts the 8 bits that it's been assigned meaning it will go on until 255 and then integer overflow error occurs
        print("the index is {} \n", .{dex});
    } 
    print("While Loop Ended\n", .{});

    // We Need To Be Careful As The While Loop will go on forever and the program will freeze if we don't break the while loop

    // We can also create a statement that will run whenever the loop iterates
    // These are called continue statements and they are an easier way to add a conditon to an condition so that the while loop breaks automatically and there is no way of a overflow happening

    var index:i8 = 0;
    while (index<15) : (index+=1) {
        print("Continue Statement Index: {}\n", .{index});
    }
}